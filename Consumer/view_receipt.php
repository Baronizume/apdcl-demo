...
if(!empty($payment['bill_no'])){

    $bill_no = mysqli_real_escape_string($conn,$payment['bill_no']);
...
if($status=="Success"){
    echo '<span class="badge bg-success fs-6">SUCCESS</span>';
}
elseif($status=="Pending"){
    echo '<span class="badge bg-warning text-dark fs-6">PENDING</span>';
}
else{
    echo '<span class="badge bg-danger fs-6">FAILED</span>';
}
?>
        ...
        <table class="table table-bordered">
             ...
             <tr>
                ...
                <td class="text-end">
                    ...
                    ₹ <?= number_format($bill['total_bill'] ?? $payment['amount'],2) ?>
